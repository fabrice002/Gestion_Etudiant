...
@section('content')
    @include('layouts.admin.sidebarGestionFilierecreate')
                ...
                @if ($departements->count() > 0)
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-12">
                                @if (isset($id))
                                    <a href="{{ route('Admin.filiere.create', $id) }}"><i class="fa fa-plus" aria-hidden="true"></i> Ajouter une filiere</a>
                                @else
                                <a href="{{ route('Admin.filiere.createFil') }}"><i class="fa fa-plus" aria-hidden="true"></i> Ajouter une filiere</a>
                                @endif
                            </div>
                            <div class="col-md-7 ">
                                <form action="{{ route('Admin.filiere.show') }}" method="get" class="row d-flex align-items-center">
                                    @csrf
                                    <div class="col-6">
                                        ...
                                        <select name="departement_id" id="" class="form-select">
                                            ...
                                            @if (isset($id))
                                                @foreach ($departements as $departement)
                                                    ...
                                                    @else
                                                        <option value="{{ $departement->id }}">{{ $departement->intitule }}
                                                        </option>
                                                    @endif
                                                @endforeach
                                            @else
