...
@section('content')
    @include('layouts.admin.sidebarGestionGroupe')
        ...
        <section class="section">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @if ($ues->count() > 0)
                        @if (isset($ue_tdSpecial_id))
                            <a href="{{ route('Admin.groupeTD.createTdSpeciale', $ue_tdSpecial_id) }}"><i class="fa fa-plus"
                                    aria-hidden="true"></i>
                                Ajouter un Td Speciale</a>
                        @else
                            @if (isset($ue_id))
                                <a href="{{ route('Admin.groupeTD.createTd', $ue_id) }}"><i class="fa fa-plus"
                                        aria-hidden="true"></i> Ajouter un TD</a>
                            @endif
                        @endif
                        <br><br>
                        <div class="row ">
                                ...
                                @if (isset($tds))
                                    @foreach ($tds as $td)
                                        <tr id="sid{{ $td->id }}">
                                            ...
                                            <td>{{ $td->ue['code'] }}</td>
                                            <td>-</td>
                                            <td><a href="{{ route('Admin.indexUE', $td->id) }}" class="btn btn-success"><i
                                ...
                                @if (isset($tdSpecials))
                                    @foreach ($tdSpecials as $tdSpecial)
                                        <tr id="sids{{ $tdSpecial->id }}">
                                            ...
                                            <td><a href="{{ route('Admin.indexUE', $tdSpecial->id) }}"
                                                    class="btn btn-success"><i class="fa fa-plus-circle"
                                                        aria-hidden="true"></i>
                                                    Voir plus</a>&ensp;
                                                <a href="javascript:void(0)" onclick="editTdSpeciale({{ $tdSpecial->id }})"
                                                    class="btn btn-danger"><i class="fa fa-edit"
                                                        aria-hidden="true"></i>Update</a>&ensp;
